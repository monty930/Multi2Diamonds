...
evalTopShape (ShapeCond _ id shE) = do
  ...
  return $
    showIdent id
      ++ " = Shape.from_cond(lambda s, h, d, c: "
      ++ shapesCode
      ++ ")\n"
...
evalShapes shapes = do
  ...
  let posStr =
        if null posCodes
          then ""
          else concatIntersperse "+" posCodes
      negStr =
        if null negCodes
          then ""
          else concatIntersperse "-" negCodes
      combinedStr =
...
evalEvValsCode [EvalVal _ val] = return $ show val
...
evalHandFeatureCode (SmartStackShape _ shape) = do
  return $
    "SmartStack(" ++ showIdent shape ++ ", Evaluator(4,3,2,1), range(0, 40))"
...
evalExp (HandAttr _ h attr) = do
  ...
  hCode <- evalHand h
  if attr `elem` lenAttr
    then do
      return $ "len(" ++ hCode ++ "." ++ showIdent attr ++ ")"
    else do
      if attr `elem` featureAttr
        then do
